...
# test convergence
thetas1 = np.loadtxt("data/pmcmc_sir/run1/thetas.csv", delimiter=",")
thetas2 = np.loadtxt("data/pmcmc_sir/run2/thetas.csv", delimiter=",")
thetas3 = np.loadtxt("data/pmcmc_sir/run3/thetas.csv", delimiter=",")

grt = gelman_rubin_test((thetas1, thetas2, thetas3))
ess = az.ess(
    az.convert_to_dataset(
        np.array([thetas1, thetas2, thetas3])
    )
)
ess = np.array(ess["x"])
length_used = (len(thetas1) - burn_in)//thinning

print(f"Gelman-Rubin: {grt}, ESS: {ess[0]}, {ess[1]} from {length_used} samples")
