...
y0 = (4800, 0, 20, 0)
parameters = [4, 1, 1]
beta, gamma, alpha = parameters
dataset = seir_simulate_discrete(y0, t, *parameters)
...
thetas_unique = np.unique(thetas, axis=0)
...
plt.legend()
plt.savefig(graphs_directory + "alpha.png", bbox_inches='tight')
plt.show()
...
sns.kdeplot(thetas3[:, 2], shade=True, label="alpha")
plt.axvline(beta)
plt.axvline(gamma)
plt.axvline(alpha)
plt.legend()
plt.savefig(graphs_directory + "hist.png", bbox_inches='tight')
plt.show()
...
res = likelihoods3 * multivariate_normal.cdf(thetas3, np.array(parameters0), h*sigma)
...
lines5 = plt.plot(range(len(data2)), dataset.iloc[:, 4], "m", linewidth=2)
lines6 = plt.plot(range(len(data2)), data2[:, 0], "b--", linewidth=2)
lines7 = plt.plot(range(len(data2)), data2[:, 1], "g--", linewidth=2)
lines8 = plt.plot(range(len(data2)), data2[:, 2], "c--", linewidth=2)
lines9 = plt.plot(range(len(data2)), data2[:, 3], "m--", linewidth=2)
plt.legend(
    lines1[:1] + lines2 + lines3 + lines4 + lines5 + lines6 + lines7 + lines8 + lines9,
    ["particle trajectories", "hidden susceptible", "hidden exposed", "hidden infected",
    "hidden removed", "observed susceptible", "observed exposed", "observed infected",
    "observed removed"],
)
...
means = [np.mean(sampled_trajs3[:, :, i], axis=1) for i in range(4)]
percentiles = [np.percentile(sampled_trajs3[:, :, i], [5, 95], axis=1) for i in range(4)]
lines1 = plt.plot(
...
for i, val in enumerate(["beta", "gamma", "alpha"]):
...
# test convergence
thetas1 = np.loadtxt("data/pmcmc_seir/run1/thetas.csv", delimiter=",")
thetas2 = np.loadtxt("data/pmcmc_seir/run2/thetas.csv", delimiter=",")
thetas3 = np.loadtxt("data/pmcmc_seir/run3/thetas.csv", delimiter=",")
...
for j, val in enumerate(["beta", "gamma", "alpha"]):
    for i in range(3):
        plt.subplot(3, 1, i+1)
        plt.plot(
            range(len(thetas3)),
