...
def abc_algo(observed_data, no_of_samples, threshold, priors):
    ...
    # trials
    max_time = observed_data['time'].iloc[-1] + 1
    ...
    for _ in range(0, no_of_samples):
        ...
        while distance > threshold:
            ...
            beta = np.random.uniform(priors['beta'][0], priors['beta'][1], size=1)[0]
            gamma = np.random.uniform(priors['gamma'][0], priors['gamma'][1], size=1)[0]
            # generate the sim data
            n_start = observed_data.loc[
                0, ["susceptible", "infected", "removed"]
            ]
            ...
            simulated_trajectories2 = np.array([[0, 0, 0]])
            ...
            distance = distance_function(
                simulated_trajectories2[:, 1],
                np.array(observed_data["infected"]),
                simulated_trajectories2[:, 2],
                np.array(observed_data["removed"]),
            )
